# -----------------------------------------------------------------------------------
# 🚀 2025 冲刺版：Parallel + SKFF(r=2) + GatedRefine(SOTA) + NoHIN
# -----------------------------------------------------------------------------------
import math
import torch
...
# ===================================================================================
# 2. 新增组件：GatedRefine (替代 CBAM) & SKFF
# ===================================================================================

# ⭐⭐⭐ 新增: Gated Refinement Unit (NAFNet Style) ⭐⭐⭐
# 原理：Gate机制是目前Restoration SOTA (如NAFNet/Restormer) 的核心
...
class GatedRefine(nn.Module):
    def __init__(self, dim):
        super().__init__()
        # 1. 门控通道混合 (Expansion = 2)
        self.project_in = nn.Conv2d(dim, dim * 2, 1)
        self.sg = SimpleGate()
        
        # 2. 深度卷积提取细节 (Replacement for Spatial Attention)
        self.dwconv = nn.Conv2d(dim, dim, 3, 1, 1, groups=dim)
        
        # 3. 简化的通道注意力 (SCA - Replacement for Channel Attention)
...
# ===================================================================================
# 5. 主干网络 (Backbone_new) - 集成 GatedRefine
# ===================================================================================
...
if __name__ == "__main__":
    print("🔍 Testing SOTA 2025 Model: Parallel + SKFF(r=2) + GatedRefine")
    model = sfhformer_t()
    ...
    # 检查 GatedRefine 是否工作
    print(f"✅ GatedRefine Check: {hasattr(model, 'refine1')}")