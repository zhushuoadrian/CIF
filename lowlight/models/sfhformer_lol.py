# -----------------------------------------------------------------------------------
# 🚀 2025 冲刺版：Parallel + SKFF(r=2) + GatedRefine(Fixed) + NoHIN
# -----------------------------------------------------------------------------------
import math
import torch
...
# ⭐⭐⭐ 修复版 GatedRefine：加入 LayerNorm 防止梯度爆炸 ⭐⭐⭐
class GatedRefine(nn.Module):
    def __init__(self, dim):
        ...
        # 5. 【修复关键】LayerNorm，稳定乘法门控的输入
        self.norm = nn.LayerNorm(dim)

    def forward(self, x):
        residual = x
        
        # 先进行 Norm (B, C, H, W) -> (B, H, W, C) -> Norm -> (B, C, H, W)
...
if __name__ == "__main__":
    print("🔍 Testing SOTA 2025 Model: Parallel + SKFF(r=2) + GatedRefine(Fixed)")
    model = sfhformer_t()
    x = torch.randn(2, 3, 64, 64)
    y = model(x)
    print(f"✅ Forward Pass: {x.shape} -> {y.shape}")
    
    # 检查 GatedRefine 是否包含 Norm
    if hasattr(model, 'refine1'):
        print(f"✅ GatedRefine LayerNorm Check: {hasattr(model.refine1, 'norm')}")