...
class VSSFreqLocalBlock(nn.Module):
    def __init__(
        self,
        hidden_dim: int = 0,
        drop_path: float = 0,
        ...
        **kwargs,
        ...
        self.fusion = nn.Conv2d(3*hidden_dim, hidden_dim, 1, 1, 0)
        
        # FFN
        if self.use_ffn:
            self.norm2 = norm_layer(hidden_dim)
            self.ffn = GDFN(dim=hidden_dim, ffn_expansion_factor=2.66, bias=False)
        else:        
            if self.mlp_branch:
    ...
    def _forward(self, input: torch.Tensor):
        ...
        x_s = self.drop_path(self.op(x_norm)).permute(0, 3, 1, 2).contiguous()
        x_local = self.local(x_norm.permute(0, 3, 1, 2).contiguous())
        
        x_freq = torch.fft.rfft2(x_norm.permute(0, 3, 1, 2).contiguous()) + 1e-8
        mag = torch.abs(x_freq)
        pha = torch.angle(x_freq)
        mag = self.frequency_block(mag)
        real = mag * torch.cos(pha)
        imag = mag * torch.sin(pha)
        x_f = torch.complex(real, imag) + 1e-8
        x_f = torch.fft.irfft2(x_f, s= tuple(x_size), norm='backward') + 1e-8
        x_f = torch.abs(x_f) + 1e-8

        
        x = self.fusion(torch.cat((x_s, x_f, x_local), dim=1)).permute(0, 2, 3, 1) + input
