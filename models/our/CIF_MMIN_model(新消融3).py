...
# =========================================================================
# 🌟 代理生成器 (Anti-Hallucination Proxy) - [保持完整]
# =========================================================================
...
# =========================================================================
# 🌟 混合方差门控 (Hybrid Variational Gate) - [保持完整自适应]
# =========================================================================
...
class OptimizedFusionClassifier(nn.Module):
    def __init__(self, input_dim, output_dim, dropout=0.3, d_state=16):
        """
        🧪 [严格消融实验版本：w/o LSDM]
        保持网络架构同构 (MultiheadAttention + Gated FFN)，
        但切断不变语义 h_invariant 的引导，退化为普通的交叉注意力。
        """
        super().__init__()
        self.input_dim = input_dim
        
        print("💡 [Fusion] 已启用严格消融模式：w/o LSDM (切断不变语义引导，退化为标准交叉注意力)。")
    ...
    def forward(self, h_real, h_imagined, h_invariant):
        # ==========================================================
        # 🚫 核心消融点：彻底无视传入的 h_invariant 变量！
        # 强制使用 h_real (即带着原始残缺属性的特征) 来生成 Query
        # ==========================================================
        Q = self.proj_q(h_real).unsqueeze(1) # [B, 1, D]
