...
# =========================================================================
# 🌟 残差保底门控 (Residual Variational Gate - 修复推理期强制注入噪声漏洞)
# =========================================================================
class HybridVariationalGate(nn.Module):
    ...
    def forward(self, feature, original_feat, hard_mask=None):
        ...
        # 动态感知缺失：如果传入的原始模态特征 L2 范数极小，说明该模态已丢失，强制将门控归零，100% 启用 Proxy
...
class CIFMMINModel(BaseModel):
    ...
    def __init__(self, opt):
        super().__init__(opt)
        # 移除原有的 KL_align，替换为余弦洗涤损失 align
    ...
    def forward(self):
        ...
        # 传入原始模态特征 feat_A_con，由门控内部自动计算 L2 范数判断有效性
    ...
    def backward(self):
        ...
        # 🌟 优化核心：基于余弦相似度的特征对齐，替代原先无意义的 KL 散度计算
        # 目的是让门控提纯后的特征向量 (Refined)，能够从方向上逼近由完整数据预训练提取的真实不变特征 (Teacher)
        ...
        # Cosine Loss 等价于 1 - Cosine Similarity
