...
# 移除第一阶段模型的依赖，端到端训练不需要 UttSelfSuperviseModel
...
# =========================================================================
# 🌟 主模型 (CIFMMINModel) - 消融版本: w/o Stage-IAnchor
# =========================================================================
class CIFMMINModel(BaseModel):
    ...
    def __init__(self, opt):
        super().__init__(opt)
        # 🌟 修改 1: 损失名称中移除 'consistent' 和 'alignment'
    ...
    def forward(self):
        ...
        self.feat_fusion_miss = torch.cat([self.feat_fusion_miss_A * w_a, self.feat_L_miss * w_l, self.feat_V_miss * w_v], dim=-1) if hasattr(self, 'feat_fusion_miss_A') else torch.cat([self.feat_A_miss * w_a, self.feat_L_miss * w_l, self.feat_V_miss * w_v], dim=-1)

        # 3. 流形重构
        self.recon_fusion, _ = self.netAE(self.feat_fusion_miss, self.consistent_miss)

        # 4. 情感分类输出
        # 🌟 修改 4: 移除 .detach()！在端到端无锚点模式下，h_invariant 必须传递梯度，
        # 从而让分类损失能够直接训练 netConA, netConL, netConV 编码器。
        self.logits, self.H_final = self.netFusionOpt(
            h_real=self.feat_fusion_miss, 
            h_imagined=self.recon_fusion, 
            h_invariant=self.consistent_miss # 删除了原代码中的 .detach()
        )

        self.pred = F.softmax(self.logits, dim=-1) if self.opt.corpus_name not in ['MOSI', 'SIMS', 'MOSEI'] else self.logits

        # ==========================================
        # 🌟 修改 5: 移除 Teacher 网络前向计算块，只保留重构所需的量级匹配无损数据生成
        # ==========================================
        if self.isTrain:
            with torch.no_grad():
                # 移除了所有与 self.pretrained_encoder 相关的计算
                
                # 提取无损的干净特征作为自编码器拟合目标
                clean_feat_A = self.netA(self.acoustic)
                clean_feat_L = self.netL(self.lexical)
                clean_feat_V = self.netV(self.visual)
                # 保持协同权重乘法，对准量级，仅用于自编码器 MSE 重构损失的目标
