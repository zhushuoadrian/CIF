...
# =========================================================================
# 🌟 混合方差门控 (Hybrid Variational Gate) - [已提取不确定性 U]
# =========================================================================
class HybridVariationalGate(nn.Module):
    ...
    def forward(self, feature, original_feat):
        ...
        # 返回 alpha, log_var 以及提取出的 uncertainty
        return alpha, log_var, uncertainty
...
class CIFMMINModel(BaseModel):
    ...
    def forward(self):
        ...
        # 2. 门控过滤与特征补偿 
        # 提取各个模态的置信度 U_a, U_l, U_v
        alpha_a, self.logvar_a, U_a = self.netgate_A(collab_feat, feat_A_con) 
        self.feat_A_refined = feat_A_con * alpha_a + proxy_A * (1 - alpha_a)

        alpha_l, self.logvar_l, U_l = self.netgate_L(collab_feat, feat_L_con)
        self.feat_L_refined = feat_L_con * alpha_l + proxy_L * (1 - alpha_l)

        alpha_v, self.logvar_v, U_v = self.netgate_V(collab_feat, feat_V_con)
        self.feat_V_refined = feat_V_con * alpha_v + proxy_V * (1 - alpha_v)

        # 🌟 保存当前 Batch 的平均不确定性，供测试阶段收集
        self.U_batch_avg = (U_a + U_l + U_v).mean() / 3.0
        ...
        # ==========================================
        # 🚨 生成对齐目标与重构的真实 Ground Truth (训练阶段)
        # ==========================================
        if self.isTrain:
            with torch.no_grad():
                # Teacher 网络的一致性表征
