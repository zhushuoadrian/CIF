...
# 虽然在此消融实验中其输出被强制清零，但保留网络结构以维持参数一致性
# =========================================================================
...
class HybridVariationalGate(nn.Module):
    ...
    def forward(self, feature, original_feat, hard_mask=None):
        ...
        # -------------------------------------------------------------
        # ✅ 遵循控制变量法：在此 w/o CMSP 消融实验中，恢复完全体的自适应门控
        # -------------------------------------------------------------
        alpha = raw_alpha 
...
class CIFMMINModel(BaseModel):
    ...
    def forward(self):
        ...
        # ====================================================
        # 🧪 【消融实验 2：w/o CMSP】
        # 强制将代理特征清零，彻底阻断跨模态协同预期补偿
        # ====================================================
        proxy_A = torch.zeros_like(proxy_A)
        proxy_V = torch.zeros_like(proxy_V)
        proxy_L = torch.zeros_like(proxy_L)
        # ====================================================
